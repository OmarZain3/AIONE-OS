...
class ProjectSerializer(serializers.ModelSerializer):
    ...
    def validate_status(self, value: str) -> str:
        ...
        if value not in valid:
            raise serializers.ValidationError(f"Invalid status. Choose from: {', '.join(sorted(valid))}.")
        return value

    def validate_color(self, value: str) -> str:
        normalized = value.strip().upper()
        if not normalized.startswith("#") or len(normalized) not in (4, 7):
            raise serializers.ValidationError("Color must be a valid hex value (e.g. #6366F1).")
        return normalized
