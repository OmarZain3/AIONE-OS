...
Thank you for contributing. This document defines the workflow and standards for all contributors.
...
2. **Branch** from `develop` using the [branch strategy](docs/standards/branch_strategy.md).
3. **Implement** changes in the appropriate top-level directory (`apps/`, `services/`, `packages/`, etc.).
4. **Commit** using [Conventional Commits](docs/standards/commit_convention.md).
5. **Open a pull request** against `develop` (or `main` for hotfixes) using the PR template.
6. **Address review feedback** — see the [code review checklist](docs/standards/code_review_checklist.md).
...
- Descriptive title matching commit convention (e.g. `feat(services): add health endpoint`).
...
Significant design decisions require an Architecture Decision Record (ADR) in [`docs/adr/`](docs/adr/). Propose the ADR in the same PR or a preceding PR before implementation.
...
Open a discussion or issue labeled `question` if workflow or standards are unclear.
